        ...
        book.view(bookId)
          .then(response => {
            ...
            category.getCategoryList()
              .then(response => {
                this.yijiFenleiList = response.data.items      //获取到数据赋值给一级分类列表 ，回显一级分类
                // 获取二级分类
                for (let i = 0; i < this.yijiFenleiList.length; i++) {    //因为一级分类包含二级分类，要想的到二级分类，需要循环一级分类
                  //判断当前的一级分类id 与 循环的 this.yijiFenleiList 一级分类列表的id 是否一致
                  if (this.book.yijiFenlei === this.yijiFenleiList[i].id) {
                    this.erjiFenleiList = this.yijiFenleiList[i].erjiFenleiList   //获取到数据赋值给二级分类列表 ，回显一级分类
                  }
                }
              })
          })
      },
      saveBook() { //修改书籍     // 点击保存按钮时
        this.saveBtnDisabled = true  // 禁用按钮
        book.updateBook(this.book)  // 调用API
          .then(response => {
            //提示信息
            this.$message({
              type: 'success',
